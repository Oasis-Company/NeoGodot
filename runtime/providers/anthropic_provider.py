...
class AnthropicProvider(BaseProvider):
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        ...
        Args:
            prompt: 输入提示词
            **kwargs: 支持 temperature, max_tokens, top_p, top_k, system,
                     stop_sequences, images 等参数
    ...
    async def generate_image(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        注意：Anthropic Claude 模型主要支持图像输入而非生成图像
        此方法主要处理图像输入场景，返回图像处理结果

        Args:
            prompt: 图像描述或问题
            **kwargs: 支持 image_url, image_base64, media_type 等参数
